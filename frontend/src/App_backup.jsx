...
function App() {
  ...
  return hotspots.filter((hotspot) => {
    const time =
      hotspot.acq_time || hotspot.acq_datetime || hotspot.acq_date

    if (!time) return false

    const hourValue = hotspot.acq_time
      ? parseInt(String(hotspot.acq_time).slice(0, 2))
  ...
  <div className="thermal-chart">
    ...
    <div className="chart">
      {thermalActivity.map((count, hour) => (
        <div className="chart-bar-container" key={hour}>
          <div
            className="chart-bar"
            style={{
              height: `${Math.max(count * 8, 4)}px`,
            }}
            title={`${count} detections at ${hour}:00`}
          ></div>
